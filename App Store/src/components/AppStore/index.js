...
class AppStore extends Component {
  state = {
    myTabId: tabsList[0].tabId,
    myText: '',
  }

  displayTabItems = tabId => {
    this.setState({myTabId: tabId})
  }
  ...
  render() {
    const {myTabId, myText} = this.state

    const filterAppItems = appsList.filter(
      eachItem => eachItem.category === myTabId,
    )

    const filterSearchAppItems = filterAppItems.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(myText.toLowerCase()),
    )
        ...
        <ul className="tab-list-container">
          {tabsList.map(eachItem => (
            <TabItem
              key={eachItem.tabId}
              myTab={eachItem}
              displayTabItems={this.displayTabItems}
              isActive={myTabId === eachItem.tabId}
            />
          ))}
        </ul>
        <ul className="app-list-container">
          {filterSearchAppItems.map(listItem => (
            <AppItem key={listItem.appId} myApp={listItem} />
          ))}
        </ul>
      </div>
    )
